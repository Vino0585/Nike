...
class MHE_Journal_Inventory:
    ...
    def create_mhe_journal_iLPN_outbound(self):
        mhe_instance = Outbound_MHE_Journal_Payload()
        raw_iLPN_payloads = mhe_instance.create_outbound_iLPN_mhe_journal_payloads()
        if not raw_iLPN_payloads:
            logging.error("No MHE Journal Payload Found")
            return None

        payloads_by_group = defaultdict(list)
        for package in raw_iLPN_payloads:
                ...
                for i, payload_to_send in enumerate(payloads):
                    try:
                        ...
                        for entry in resp:
                            ...
                            elif events == 'REPLEN_TASK_COMPLETED':
                                goodsholder_id = message_payload_dict['data']['sourceGoodsholderId']
                            elif events == 'PICK_TASK_COMPLETED':
                                goodsholder_id = message_payload_dict['data']['goodsholderId']
                            ...
                            substituted_ilpn = None
                            if events == 'RETRIEVAL_TASK_COMPLETED':
                                substituted_ilpn = message_payload_dict['data']['requestedGoodsholderId']
                            ...
                            if entry.get('Status') != 'NO DESTINATION FOUND':
                                if entry.get('Status') != 'SPLIT CREATED':
                                    result_row = {
                                        ...
                                        'Substitute_From_iLPN': substituted_ilpn if events == 'RETRIEVAL_TASK_COMPLETED' else '',
                                        'Message_Type': entry.get('MessageType') if entry.get('MessageType') != 'PTW_DEI_AllocationCreated' else 'EXECUTE_REPLEN_TASK',
        ...
        try:
            ...
            results_df = results_df.sort_values(by=['LPN_ID'])
    ...
    def create_mhe_journal_oLPN_outbound(self):
            mhe_instance = Outbound_MHE_Journal_Payload()
            raw_oLPN_payloads = mhe_instance.create_outbound_oLPN_mhe_journal_payloads()
            if not raw_oLPN_payloads:
                logging.error("No MHE Journal Payload Found")
                return None

            payloads_by_group = defaultdict(list)
            for package in raw_oLPN_payloads:
                if not isinstance(package, dict):
                    logging.error(f"WARNING: Skipping package as it's not a valid dictionary: {package}")
                    continue
                    ...
                    for i, payload_to_send in enumerate(payloads):
                        try:
                            ...
                            for entry in resp:
                                ...
                                # message_types = ['PPK_DEI_TaskRelease', 'PickTaskResult', 'Pack_Complete',
                                #                  'GoodsholderMeasured',
                                #                  'RoutingTaskResult']

                                if events in ('GOODSHOLDER_MEASURED', 'ROUTING_TASK_COMPLETED'):
                                    goodsholder_id = message_payload_dict['data']['goodsholderId']
                                elif events == 'PPK_DEI_TaskRelease':
                                    task_data = message_payload_dict.get("TaskData", {})
                                    data_content = task_data.get("data")
                                ...
                                spur_id = ''
                                if events == 'ROUTING_TASK_COMPLETED':
                                    spur_id = message_payload_dict['data']['routingTaskCompleted']['routedToDestinationLocationId']
                                ...
                                if entry.get('Status') != 'NO DESTINATION FOUND':
                                    if entry.get('Status') != 'SPLIT CREATED':
                                        result_row = {
                                            ...
                                            'MessageID': entry.get('MessageId'),
                                            'LPN_ID': goodsholder_id,
                                            'Spur_ID': spur_id if events == 'ROUTING_TASK_COMPLETED' else '',
...
if __name__ == "__main__":
    ...
    mhe_journal.create_mhe_journal_oLPN_outbound()